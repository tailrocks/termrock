...
#[test]
fn explicit_safe_confirmation_focuses_yes_without_changing_destructive_default() {
    let exit = ConfirmState::new("Exit?").with_focus_yes();
...
fn confirm_button_hit_matches_data_loss_rendered_buttons() {
    ...
    let s = ConfirmState::details(
        ...
        "Exit?",
        Vec::new(),
        vec!["Unsaved work will be lost.".into()],
    );
