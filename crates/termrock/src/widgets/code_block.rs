...
impl<'a, H: SyntaxHighlighter> CodeBlock<'a, H> {
    ...
    pub fn paint(
        ...
        while row < body_h {
            ...
            // Catalog s_editor goldens that want a bar on every row must fill
            // the capture, not this widget: showcase crops have space on
            // off-cursor lines.
            if parts.gutter.width > 0 {
                ...
                // space, then the `num_w` column). Diagnostic bang still
                // overwrites the last number cell.
