...
**Burn-down completed 2026-08-16 (plans/019).** The showcase exists, runs, and
has no open API gap. Missing generic capabilities shipped in TermRock;
product projections stayed in the host. GAP-WB-2 closed the way the SoT said
it could: with a
...
| GAP-CM-1 | Context only TokenMeter | ContextMeter | P1 | agent elevate | G5c | **closed by correct ownership** — `TokenMeter` covers the showcase's measured need; no distinct generic capability or behavior exists to justify a duplicate ContextMeter |
