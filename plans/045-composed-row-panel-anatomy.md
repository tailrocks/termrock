...
- **Execution**: DONE — ComposedRow paint path on List/Tree/Table + PanelSlots
...
- [x] Hidden/disabled parts cannot advertise or trigger actions. (row-level hits; disabled rows skipped)
- [x] List/Tree/Table/Panel reuse one anatomy/recipe grammar (`ComposedRow` / `PanelSlots`).
- [x] Migration `0039` (composed anatomy; plan text historically said 0038 — intents took 0038).
- [x] Old label+trailing-only paint path removed; adapters keep `ListRow`/`TreeNode` as projection types.
